-- FUNCTION: public.school_history_trigger()

-- DROP FUNCTION IF EXISTS public.school_history_trigger();

CREATE OR REPLACE FUNCTION public.school_history_trigger()
    RETURNS trigger
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
    ...
    ELSIF (TG_OP = 'INSERT') THEN
        INSERT INTO changes_log (pk, table_title, field, old_value, new_value, user_name, change_time, change_date, change_type)
        VALUES (NEW.id, 'report', 'report', NULL, NEW.fk_group, CURRENT_USER, NOW(), CURRENT_DATE, 'insert');
    END IF;
    RETURN NEW;
END 
$BODY$;

ALTER FUNCTION public.school_history_trigger()
    OWNER TO postgres;
